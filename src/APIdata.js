...
// getAll() returns all books (array)
//  books.map((book => (
//     <div>
//         {book.id} {book.imageLinks.thumbnail}  {book.title, book.authors}
//     </div>
// ))
// shelf: 'wantToRead', 'read', 'currentlyReading'
const getAllbooks =     
    [
        {
...
// get(bookId) returns one book (object)
const getABook =  {
    "title": "Satire TV",
    "subtitle": "Politics and Comedy in the Post-Network Era",
    "authors": [
...
// To move a book between shelves  
// update(book, shelf)
...
const update = (book, shelf) =>
  ...
  //query: <String>
  // search(query) returns an array of books that match query 
 const search = (query) =>
  fetch(`${api}/search`, {
    method: 'POST',
    headers: {
